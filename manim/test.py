...
def update_path(mob, dt, tracers, circle_center):
    global frame_count
    if frame_count != 0 : # without this line we get a 0 that won't go away
        if (frame_count % 5 == 0) :
            while len(tracers) > 4 :
                oldtracer = tracers.pop(0)
                print(f"Tracer popped and mob removed: {oldtracer}")
            ...
            print(f"Tracer appended and mob added: {tracer}")
            mob.add(tracer)
