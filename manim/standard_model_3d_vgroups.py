        ...
        for i, A in enumerate(dictionary_of_assemblies):
            ...
            if do_noether_core:
                for c, charge in enumerate(charges_in_the_noether_core):
                    ...
                    if (c in [2, 3]) and generation == 1:
                        continue
                    elif (c in [4, 5]) and (generation in [1, 2]):
                        continue
                    ...
                    if (c % 2) == 0:
                        z = 0
                        kwargs['stroke_opacity'] = 0 #don't show this one. it is here just so we can start the charge at PI offset.
                        orbital_path = Circle(**kwargs)
                        orbital_path.rotate_about_origin(PI)
                    else: 
                        z = 0.5
                        orbital_path = Circle(**kwargs)
                    ...
                    assemblies.append(MoveAlongPath(sphere, orbital_path, radians=TAU*charge['orbit_cycles'], rate_func=linear, run_time=run_time ))
            ...
            if do_personality:
                ...
                for p, personality in enumerate(personality_charges):
                    ...
                    self.add(dot)
                    assemblies.append(MoveAlongPath(dot, personality_orbital_path, radians=TAU*32, rate_func=linear, run_time=run_time ))
